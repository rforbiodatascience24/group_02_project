...
We want to make a heat map of the world, so we need an additional library and join that to the data.
...
# Join to excisting data
manufacturer_world_pop <- world_map |> 
  left_join(manufacturer_world_pop, by = c("region" = "Country"))
```
